...
void main() {
  group('Mock Authentication', () {
    ...
    test('Create user should delegate to logIn function', () async {
      final badEmailUser =
          provider.createUser(email: 'nice@foo.com', password: 'foobar');
      expect(badEmailUser,
          throwsA(const TypeMatcher<UserNotFoundAuthException>()));

      final badPasswordUser =
          provider.createUser(email: 'email@google.com', password: 'foobar');
...
class MockAuthProvider implements AuthProvider {
  ...
  Future<AuthUser> logIn({
    ...
    if (!isIntialized) throw NotInitializedException();
    if (email == 'nice@foo.com') throw UserNotFoundAuthException();
    if (password == "foobar") throw WrongPasswordAuthException();
    const user = AuthUser(isEmailVerified: false, email: 'nice@foo.com');
    _user = user;
  ...
  Future<void> sendEmailVerificaiton() async {
    ...
    const newUser = AuthUser(isEmailVerified: true, email: 'nice@foo.com');
    _user = newUser;
  }
}
